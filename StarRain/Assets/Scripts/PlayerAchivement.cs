...
public class PlayerAchivement : MonoBehaviour
{
    ...
    void Start()
    {
        backgorund_index = PlayerPrefs.GetInt("PlayerID");
        updateWindow();
        clearRequirements();
    }
    ...
    private void fillRequirements(){
        ...
        requirementProgress.GetComponent<Text>().text = AchievmentMeasures.get_requirement_status("unlockPlayer" + (backgorund_index+1).ToString()) + " /";
        requirement.SetActive(true);
        return;
    }
    private bool check_requirements(){
        if( requirements_targets[backgorund_index] == 0.0f ) return true;
        return AchievmentMeasures.is_requirement_meet( "unlockPlayer" + (backgorund_index+1).ToString(), requirements_targets[backgorund_index] );
    }
