...
public class HolderController : MonoBehaviour
{
    ...
    private void SetName(){
        transform.GetChild(0).GetChild(1).GetChild(0).GetComponent<Text>().text = objectName;
    }

    private void DisableRequiremenetField(){
        var requirementsPart = transform.GetChild(0).GetChild(2);
    ...
    void shiftToSide( ){
        ...
        if( shiftToRight ){
            newPostion.x  += 100.0f-Mathf.Abs(parentPosiiton.y - index * DISTANCE);
        }else{
            newPostion.x  += -100.0f + Mathf.Abs(parentPosiiton.y - index * DISTANCE) ;
        }
    ...
    private void changeVisibilityNameField( Transform buttonBackgound, float alphaRate ){
        var namePart = buttonBackgound.GetChild(1);
    ...
    private void changeVisibilityRequirementField( Transform buttonBackgound, float alphaRate ){
        var requirementsPart = buttonBackgound.GetChild(2);
        requirementsPart.GetComponent<Image>().color = new Color(1,1,1, Mathf.Min(145.0f/255.0f, alphaRate));
        for( int i = 0; i < requirementsPart.childCount; i++){
